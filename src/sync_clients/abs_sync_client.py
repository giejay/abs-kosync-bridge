...
class ABSSyncClient(SyncClient):
    ...
    def update_progress(self, book: Book, request: UpdateProgressRequest) -> SyncResult:
        ...
        if ts_for_text is not None:
            ...
            abs_ts = response.get('currentTime') if response is not None else None
            if abs_ts is not None and ts_for_text < abs_ts and not request.allow_backtrack:
            ...
            if abs_ts is not None and ts_for_text < abs_ts and request.allow_backtrack:
                logger.info(f"[{book_title}] Backtracking allowed - updating ABS from {abs_ts:.2f}s to {ts_for_text:.2f}s.")
